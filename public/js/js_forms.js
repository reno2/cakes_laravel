...
//window.onload=function(){
    ...
    let imgInput = document.querySelector('.js_fileInput')
    if (imgInput) {
...
const addToValidArray = function(file){
    if(tmpForMultipleFiles.length > 0){
        let newFile = true;
        for (const element of tmpForMultipleFiles) {
            // проверяем что файлта там нет
            if(element.name === file.name) newFile = false
        }
        if(newFile) tmpForMultipleFiles.push(file)
    }else{
        tmpForMultipleFiles.push(file)
    }
}

// Убераем сообщения об ошибках
const removeNotice = function(el) {
    if (el.nextSibling)
...
const validateRule =  {
    limit: (file, el) => {
        ...
        if (tmpForMultipleFiles.length + 1 > 5) {
            showNotice(el, msg)
            return false
        }
        return true
